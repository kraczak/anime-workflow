...
class AnimeModel(BaseModel):
    ...
    def get_all(filters: List[peewee.Expression] = None, op=operator.and_):
        if filters is None:
            return AnimeModel.select().execute()
        return AnimeModel.select().where(reduce(op, filters)).execute()

    @staticmethod
    def get_followed(filters: List[peewee.Expression] = None, op=operator.and_):
        if filters is None:
            filters = []
        tmp_filters = [AnimeModel.follow == 1] + filters
        return AnimeModel.select().where(reduce(op, tmp_filters)).execute()

    @staticmethod
    def get_not_followed(filters: List[peewee.Expression] = None, op=operator.and_):
        if filters is None:
            filters = []
        tmp_filters = [AnimeModel.follow == 0] + filters
        return AnimeModel.select().where(reduce(op, tmp_filters)).execute()
